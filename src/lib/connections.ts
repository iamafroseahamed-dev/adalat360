...
export async function loadConnections(caseId: string): Promise<ConnectedCaseRow[]> {
  ...
  const { data: cs } = await supabase
    .from('cases')
    .select('id, case_number, court_name, case_status, next_hearing_date')
    .in('id', otherIds);
...
export async function addConnection(parentId: string, connectedId: string, relationshipType: string) {
...
export async function loadConnectionCounts(): Promise<Record<string, number>> {
